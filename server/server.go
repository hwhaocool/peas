...
func echoServer() (*tcpnetwork.TCPNetwork, error) {
	...
	log.Println("server listen at ", kServerAddress)
...
func routineInput(wg *sync.WaitGroup, server *tcpnetwork.TCPNetwork) {
	defer func() {
		log.Println("server input done")
		wg.Done()
	}()
	...
	for {
		...
		num := server.SendData2AllClients(str)

		log.Println("send data ->", str, "<- to ", num, " clients success")
	}
}
